...
async function main() {
  try {
    console.log('Starting additional lease template creation...');
...
Tenant: __________________________ Date: _____________`,
    ...
    console.log(`Created month-to-month lease template: ${monthToMonthTemplate.id}`);
...
Tenant: __________________________ Date: _____________`,
    ...
    // Create vacation rental agreement template
    const vacationRentalTemplate = await prisma.leaseTemplate.create({
      data: {
        name: 'Vacation Rental Agreement',
        description: 'Short-term rental agreement for vacation properties',
        userId: landlord.id,
        content: `# VACATION RENTAL AGREEMENT

This Vacation Rental Agreement ("Agreement") is made and entered into on {CURRENT_DATE}, by and between the Landlord and Tenant as specified below.
...
The Landlord hereby rents to the Tenant, and the Tenant hereby rents from the Landlord, the vacation property located at:
...
This Agreement shall commence on {LEASE_START_DATE} at 3:00 PM and shall end on {LEASE_END_DATE} at 11:00 AM ("Rental Term").

## 4. RENTAL AMOUNT

Tenant agrees to pay {MONTHLY_RENT} as the total rental amount for the entire Rental Term, payable as follows:
- 50% due at the time of booking
- Remaining 50% due 30 days prior to check-in
...
The maximum number of guests is limited to 6 persons. Additional guests without prior approval will be subject to immediate termination of this Agreement without refund.

## 7. PETS

No pets are allowed on the Premises without prior written approval of Landlord.

## 8. HOUSE RULES

Tenant agrees to abide by the House Rules attached to this Agreement.

## 9. CANCELLATION POLICY

- Cancellation more than 60 days prior to check-in: Full refund minus $100 administrative fee
- Cancellation 30-60 days prior to check-in: 50% refund
- Cancellation less than 30 days prior to check-in: No refund
...
Tenant: __________________________ Date: _____________`,
    ...
    console.log(`Created vacation rental template: ${vacationRentalTemplate.id}`);

    console.log('Additional lease templates created successfully!');
  } catch (error) {
    console.error('Error creating additional lease templates:', error);
  } finally {
