...
import { CreditCard, Wrench, FileText, Bell, Home, Calendar } from "lucide-react"
...
export default function TenantDashboard() {
  ...
  const daysUntilRentDue = tenantData?.rentDue 
    ? Math.ceil((new Date(tenantData.rentDue).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
    : 0
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="flex flex-col space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Due in {daysUntilRentDue} days</span>
                    <span className="text-primary-foreground">
                      {tenantData?.rentDue ? new Date(tenantData.rentDue).toLocaleDateString() : "N/A"}
                    </span>
                  </div>
                  <Progress value={Math.max(0, Math.min(100, 100 - (daysUntilRentDue * 3.33)))} />
                </div>
              ...
              <div className="flex flex-col space-y-4">
                <div className="p-4 rounded-lg bg-muted/50 border border-border">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-primary-foreground">Active Requests</span>
                    <span className="text-sm bg-primary/20 text-primary px-2 py-1 rounded-full">
                      {tenantData?.maintenanceRequests?.filter((r: any) => r.status !== "completed").length || 0}
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    {tenantData?.maintenanceRequests?.filter((r: any) => r.status !== "completed").length 
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          <CardTitle className="text-primary-foreground">Property Information</CardTitle>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="text-lg font-medium text-primary-foreground mb-2">Address</h3>
                <p className="text-muted-foreground">
                  {tenantData?.property?.address || "N/A"}<br />
                  {tenantData?.property?.city || "N/A"}, {tenantData?.property?.state || "N/A"} {tenantData?.property?.zipCode || "N/A"}
                </p>
              </div>
              <div>
                <h3 className="text-lg font-medium text-primary-foreground mb-2">Lease Information</h3>
                <div className="space-y-1">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Start Date:</span>
                    <span className="text-primary-foreground">
                      {tenantData?.leaseStart ? new Date(tenantData.leaseStart).toLocaleDateString() : "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-primary-foreground">${tenantData?.rentAmount || "0.00"}</span>
                  </div>
                </div>
              </div>
            </div>
