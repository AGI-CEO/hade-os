...
import { Search, Filter, Plus, MessageSquare, FileText, UserPlus, Smile } from "lucide-react"
...
export default function TenantsPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<"all" | "active" | "pending" | "past">("all")
  const [showAddTenant, setShowAddTenant] = useState(false)

  return (
    <div className="space-y-6">
      <div className="flex flex-col space-y-2">
        <h1 className="text-3xl font-bold tracking-tight text-primary-foreground glow-text">Tenants</h1>
        ...
        <Tabs defaultValue="all" className="w-full">
          ...
          <div className="mt-6">
            <AnimatePresence mode="wait">
              ...
              >
                <TenantsList filterStatus={filterStatus} searchQuery={searchQuery} />
              </motion.div>
            </AnimatePresence>
          </div>
        </Tabs>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-card border-border md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Monitor tenant satisfaction and take action to improve it</CardDescription>
          </CardHeader>
          <CardContent>
            <TenantHappiness />
          </CardContent>
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3 p-3 rounded-lg bg-card/50 border border-border">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    <h4 className="font-medium text-primary-foreground">John Smith</h4>
                    <span className="text-xs text-muted-foreground">2h ago</span>
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">Requested maintenance for kitchen sink</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3 rounded-lg bg-card/50 border border-border">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Rent payment confirmed for May</p>
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            ...
            <CardDescription>Screen potential tenants with AI assistance</CardDescription>
          </CardHeader>
          <CardContent>
            <TenantResearch />
          </CardContent>
      ...
      <AnimatePresence>{showAddTenant && <AddTenantModal onClose={() => setShowAddTenant(false)} />}</AnimatePresence>
    </div>
  )
}
...
function AddTenantModal({ onClose }: { onClose: () => void }) {
  return (
    ...
    >
      ...
      >
        <div className="p-6">
          <h2 className="text-xl font-bold text-primary-foreground mb-4">Add New Tenant</h2>
          <p className="text-muted-foreground mb-6">Enter tenant details or use AI to screen potential tenants</p>

          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-primary-foreground">First Name</label>
                <Input placeholder="John" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-primary-foreground">Last Name</label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-primary-foreground">Phone</label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-primary-foreground">Lease Start Date</label>
              <Input type="date" />
            </div>
          </div>
